...
def run_simulation_case(netlist_path, current_log_path, voltage_log_path):
    ...
    1. **Identify the Trojan lines**: Based on the Trojan circuit shown in the above example, analyze the new SPICE netlist shown below  and check if you can detect similar Trojan lines. List the Trojan lines in this specific format: 
    "Trojan: 
    line 1, line 2...
    **End.**"
...
def rerun_spice_analysis(updated_netlist_file, hspice_log_file):
    ...
    **End.**"
    ...
    Desired output voltage specification:
    The voltage at node '5' should be greater than 1.4V for vin between 0V and 1.8V.
    """
...
def decide_llm_action(updated_netlist_file, hspice_log_file):
    ...
    You will be provided with the updated SPICE netlist and its corresponding simulation log file, and the desired specifications for the primary output voltage behavior. Your task is to check the voltage at node '5' and determine if it outside the voltage specifications. Your answer should be just one word: "Continue", "Undo", or "Exit". Follow the instructions below.
    1. Output "Continue" if voltage at node '5' remains outside the specification and voltage at other nodes do not degrade further.
    2. Output "Undo" if voltage at node '5' remains outside the specification and voltage at other nodes degrade significantly.
    3. Output "Exit" if voltage at node '5' remains inside the specification.

    Netlist:
    {netlist}

    Voltage Simulation Log:
    {voltage_log}
...
def prune_lines_and_run_hspice(netlist_path, output_file_path):
    ...
    previous_netlist_lines = netlist_lines[:]
    flag = 0 
    ...
    for idx, line_to_remove in enumerate(lines_to_remove):
        ...
        # Create a unique updated netlist file
        updated_netlist_file = f"updated_netlist_{idx}.sp"
        with open(updated_netlist_file, 'w') as f_updated:
            f_updated.writelines(updated_netlist_lines)

        # Create a unique HSPICE output log file for each iteration
        ...
        if response1 == "Undo":
            print("Reverting to the previous netlist.")
            netlist_lines = previous_netlist_lines[:]

        if response1 == "Exit":
            print("Netlist is Trojan-free. Woohoo!!")
            flag = 1
            break
...
for case_id in range(4, 5):
    ...
    if netlist_path and current_log_path and voltage_log_path:
        ...
        for i in range(1, 5):
            if flag == 1:
                break  # Break if Trojan-free response is found
...
print(f"Total time taken for response generation: {response_end_time - response_start_time} seconds")
